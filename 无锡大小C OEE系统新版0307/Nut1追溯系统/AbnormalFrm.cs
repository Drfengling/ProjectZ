    ...
    public partial class AbnormalFrm : Form
    {
        ...
        private void btn_LogQuery_Click(object sender, EventArgs e)//日志信息查询
        {
            try
            {
                ...
                if ((dtp_LogEndTime.Value - dtp_LogStartTime.Value).TotalSeconds > 0)
                {
                    ...
                    if (days == 0)
                    {
                        if (File.Exists(@"D:\ZHH\操作记录\" + dtp_LogStartTime.Value.Date.AddDays(days).ToString("yyyyMMdd") + ".csv"))
                        {
                            FileStream fs = new FileStream(@"D:\ZHH\操作记录\" + dtp_LogStartTime.Value.Date.AddDays(days).ToString("yyyyMMdd") + ".csv", FileMode.Open, FileAccess.Read);
                    ...
                    else
                    {
                        for (int t = 0; t <= days; t++)
                        {
                            ...
                            {
                                if (File.Exists(@"D:\ZHH\操作记录\" + dtp_LogStartTime.Value.Date.AddDays(t).ToString("yyyyMMdd") + ".csv"))
                                {
                                    FileStream fs = new FileStream(@"D:\ZHH\操作记录\" + dtp_LogStartTime.Value.Date.AddDays(t).ToString("yyyyMMdd") + ".csv", FileMode.Open, FileAccess.Read);
                                    using (StreamReader sr = new StreamReader(fs, Encoding.Default))
                                    {
                                        string strline = "";
                                        string[] anyline = null;
        ...
        private void btn_LogExport_Click(object sender, EventArgs e)
        {
            ...
            if (sf.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                foreach (var item in lst_AbnormalMes.Items)
                {
