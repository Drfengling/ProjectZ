    ...
    public partial class SettingFrm : Form
    {
        ...
        private void SettingFrm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                txtURL_Process_LA.Text = Global.inidata.productconfig.Trace_CheakSN_Bracket;
        ...
        private void SaveSoftwareParameter()
        {
            ...
            for (int i = 0; i < _name.Count; i++)
            {
                ...
                {
                    Log.WriteLog(Global.Name + " 工号：" + Global.Emp + "  " + Global._listName[i] + "参数修改：" + Global._listValue[i] + "→" + _value[i]);
                    Log.WriteCSV(String.Format("{0},{1},{2},{3},{4},{5}", Global.Name, Global.Title, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"), Global._listName[i], Global._listValue[i], _value[i]), System.AppDomain.CurrentDomain.BaseDirectory + "\\参数修改记录\\");
                }
            }
