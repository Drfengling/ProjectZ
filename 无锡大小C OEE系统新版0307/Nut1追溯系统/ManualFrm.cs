...
namespace 卓汇数据追溯系统
{
    public partial class ManualFrm : Form
    {
        ...
        private void CB_errorinfo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (CB_errorinfo.SelectedItem.ToString().Contains("吃饭休息(M)"))
                {
                    LB_ErrorCode.Text = "[11010001,7]";
                }
                ...
                if (CB_errorinfo.SelectedItem.ToString().Contains("其他原因调试(M)"))
                {
                    LB_ErrorCode.Text = "[70020006,5]";
                }
                if (CB_errorinfo.SelectedItem.ToString().Contains("镭焊机故障(M)"))
                {
        ...
        private void btn_errortime_Click(object sender, EventArgs e)
        {
            ...
            //        string ts = (t2 - t1).TotalMinutes.ToString("0.00");
            //        Log.WriteCSV(DateTime.Now.ToString("HH:mm:ss") + "," + Global.ed[Global.Error_num].errorCode + "," + Global.ed[Global.Error_num].start_time + "," + Global.ed[Global.Error_num].start_time + "," + "自动发送成功" + "," + Global.ed[Global.Error_num].errorStatus + "," + Global.ed[Global.Error_num].errorinfo + "," + ts, @"F:\装机软件\系统配置\System_ini\");
            //    }
            //    string OEE_DT = "";
            //    string msg = "";
        ...
        private void btnManualOEEStatus_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Global.PLC_Client.WritePLC_D(23030, new short[] { 1 });//手动选择打开安全门原因，机台可以运行
                //ButtonFlag(false, 0);
            }
        }
        ...
        private void txtSelectSN_TextChanged(object sender, EventArgs e)
        {
            if (txtSelectSN.Text.Length == 19)
            {
                ...
                string SelectStr = string.Format("SELECT * FROM Trace_UA_SendNG WHERE sp='{0}'", serial_number);//sql查询语句
                DataTable d1 = SQL.ExecuteQuery(SelectStr);
                string SelectStr2 = string.Format("SELECT * FROM PDCA_SendNG WHERE sp='{0}'", serial_number);//sql查询语句
        ...
        private void btn_UpLoad_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgv_TraceUASendNG.Rows.Count > 0)//Trace_UA上传失败缓存
                {
                    ...
                    TraceData.serials.band = dgv_TraceUASendNG.Rows[0].Cells[2].EditedFormattedValue.ToString();
                    TraceData.data.insight.test_attributes.test_result = dgv_TraceUASendNG.Rows[0].Cells[3].EditedFormattedValue.ToString();
                    TraceData.data.insight.test_attributes.unit_serial_number = TraceData.serials.band.Remove(17);
                    TraceData.data.insight.uut_attributes.band_sn = TraceData.serials.band;
                    TraceData.data.insight.test_station_attributes.fixture_id = dgv_TraceUASendNG.Rows[0].Cells[4].EditedFormattedValue.ToString();
                    TraceData.data.insight.uut_attributes.fixture_id = dgv_TraceUASendNG.Rows[0].Cells[4].EditedFormattedValue.ToString();
                    TraceData.data.insight.uut_attributes.tossing_item = dgv_TraceUASendNG.Rows[0].Cells[5].EditedFormattedValue.ToString();
                    TraceData.data.insight.uut_attributes.STATION_STRING = dgv_TraceUASendNG.Rows[0].Cells[6].EditedFormattedValue.ToString();
                    TraceData.data.insight.test_attributes.uut_start = dgv_TraceUASendNG.Rows[0].Cells[7].EditedFormattedValue.ToString();
                    TraceData.data.insight.uut_attributes.weld_start_time = dgv_TraceUASendNG.Rows[0].Cells[8].EditedFormattedValue.ToString();
                    TraceData.data.insight.uut_attributes.weld_stop_time = dgv_TraceUASendNG.Rows[0].Cells[9].EditedFormattedValue.ToString();
                    TraceData.data.insight.test_attributes.uut_stop = dgv_TraceUASendNG.Rows[0].Cells[10].EditedFormattedValue.ToString();
                    _mainparent.Out_Trace_ua.Add(TraceData.serials.band);
                    _mainparent.Trace_ua.Add(TraceData.serials.band, TraceData);
                    lstSelectSN.Items.Add(string.Format("手动上传TraceUA-{0}缓存记录！", TraceData.serials.band));
                    Log.WriteLog(string.Format("手动上传TraceUA-{0}缓存记录！", TraceData.serials.band));
                    string DeleteStr = string.Format("delete from Trace_UA_SendNG where sp = '{0}'", TraceData.serials.band);
                    SQL.ExecuteUpdate(DeleteStr);
                    Thread.Sleep(1000);
                    string SelectStr = string.Format("SELECT * FROM Trace_UA_SendNG where sp = '{0}'", TraceData.serials.band);//sql查询语句
                    DataTable d1 = SQL.ExecuteQuery(SelectStr);
                    _mainparent.ShowData(dgv_TraceUASendNG, d1, 0);
                    _mainparent.dgv_AutoSize(dgv_TraceUASendNG);
                    TraceData = null;
                ...
                if (dgv_PDCASendNG.Rows.Count > 0)//PDCA_UA上传失败缓存
                {
                    ...
                    string DeleteStr = string.Format("delete from PDCA_SendNG where sp = '{0}'", bail.full_sn);
                    SQL.ExecuteUpdate(DeleteStr);
                    Thread.Sleep(1000);
                    string SelectStr = string.Format("SELECT * FROM PDCA_SendNG where sp = '{0}'", bail.full_sn);//sql查询语句
                    DataTable d1 = SQL.ExecuteQuery(SelectStr);
        ...
        private void Btn_Start_errortime_Click(object sender, EventArgs e)//首件开始
        {
            try
            {
                ...
                var IP = GetIp();
                var Mac = GetMac();
                short[] ReadTestRunStatus = Global.PLC_Client.ReadPLC_D(11400, 1);
                if (ReadTestRunStatus[0] != 1)//判断是否处于空跑状态（PLC屏蔽部分功能如：安全门，扫码枪，机械手）
                {
                    if (!Global.BreakStatus)//不是吃饭休息时
                    {
                        ///20210926
                        /// 
                        string str = "SELECT TOP 1 [RunStatus]FROM [ZHH].[dbo].[OEE_DT] order by DateTime desc";
                        DataTable dt = SQL.ExecuteQuery(str);
                        int lastStatus = -1;
                        if (dt != null) {
                            lastStatus =int.Parse( dt.Rows[0][0].ToString());
                        }

                        if (Global.j == 1 && Global.j!=lastStatus)//处于待料状态
                        {
                        ...
                        else if (Global.j == 3)
                        {
                            ...
                            DateTime t2 = Convert.ToDateTime(Global.ed[Global.Error_num].stop_time);
                            string ts = (t2 - t1).TotalMinutes.ToString("0.00");
                            if (Global.Error_num == 9 || Global.Error_num == 10 || Global.Error_num == 11 || Global.Error_num == 12
                                || Global.Error_num == 53 || Global.Error_num == 295 || Global.Error_num == 296 || Global.Error_num == 297
                                || Global.Error_num == 299 || Global.Error_num == 300 || Global.Error_num == 301 || Global.Error_num == 302
                                || Global.Error_num == 303 || Global.Error_num == 304 || Global.Error_num == 305 || Global.Error_num == 306)//机台打开安全门
                            {
                                string OEE_DT2 = "";
                                string msg2 = "";
                               ...
                               MainFrm.Goee?.UploadDowntime(poorNum1, TotalNum1, Global.errorStatus, Global.errorcode, Global.ed[Global.Error_num].ModuleCode,false,date, Global.errorinfo + ",安全门打开");
                                ///0924
                                ...
                                //if (rst2)
                                //{
                                //    _mainparent._homefrm.AppendRichText(Global.errorcode + ",触发时间=" + Global.ed[Global.Error_num].start_time + ",运行状态:" + Global.errorStatus + ",故障描述:" + Global.errorinfo + ",安全门打开自动发送成功", "rtx_DownTimeMsg");
                                //    Log.WriteLog("OEE_DT安全门打开自动errorCode发送成功");
                                //    Global.ConnectOEEFlag = true;
                                //}
                                //else
                                //{
                                //    _mainparent._homefrm.AppendRichText(Global.errorcode + ",触发时间=" + Global.ed[Global.Error_num].start_time + ",运行状态:" + Global.errorStatus + ",故障描述:" + Global.errorinfo + ",安全门打开自动发送失败", "rtx_DownTimeMsg");
                                ...
                                string InsertOEEStr = "insert into OEE_DT([DateTime],[ErrorCode],[EventTime],[ModuleCode],[RunStatus],[ErrorInfo],[TimeSpan])" + " " + "values(" + "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "'" + "," + "'" + Global.errorcode + "'" + "," + "'" + Global.ed[Global.Error_num].start_time + "'" + ","
                        ...
                        else if (Global.j == 4)//处于人工停止状态
                        {
                            ...
                            MainFrm.Goee?.UploadDowntime(poorNum2, TotalNum2, Global.ed[Global.Error_Stopnum].errorStatus, Global.ed[Global.Error_Stopnum].errorCode, Global.ed[Global.Error_Stopnum].ModuleCode,false,date, Global.ed[Global.Error_Stopnum].errorinfo + ",人工停止复位");
                            ///0924


                            //if (rst2)
                            //{
                            //    _mainparent._homefrm.AppendRichText(Global.ed[Global.Error_Stopnum].errorCode + ",触发时间=" + Global.ed[Global.Error_Stopnum].start_time + ",运行状态:" + Global.ed[Global.Error_Stopnum].errorStatus + ",故障描述:" + Global.ed[Global.Error_Stopnum].errorinfo + ",人工停止复位自动发送成功", "rtx_DownTimeMsg");
                            //    Log.WriteLog("OEE_DT人工停止复位自动errorCode发送成功");
                            //    Global.ConnectOEEFlag = true;
                            //}
                            //else
                            //{
                            //    _mainparent._homefrm.AppendRichText(Global.ed[Global.Error_Stopnum].errorCode + ",触发时间=" + Global.ed[Global.Error_Stopnum].start_time + ",运行状态:" + Global.ed[Global.Error_Stopnum].errorStatus + ",故障描述:" + Global.ed[Global.Error_Stopnum].errorinfo + ",人工停止复位自动发送失败", "rtx_DownTimeMsg");
                            ...
                            string InsertOEEStr = "insert into OEE_DT([DateTime],[ErrorCode],[EventTime],[ModuleCode],[RunStatus],[ErrorInfo],[TimeSpan])" + " " + "values(" + "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "'" + "," + "'" + Global.ed[Global.Error_Stopnum].errorCode + "'" + "," + "'" + Global.ed[Global.Error_Stopnum].start_time + "'" + ","
                ...
                {
                    if (Global.SelectTestRunModel == true && Global.ed[280].start_time != null)//空运行结束写入OEE_DT数据表中
                    {
                        Global.ed[280].stop_time = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff");
                        DateTime t1 = Convert.ToDateTime(Global.ed[280].start_time);
                        DateTime t2 = Convert.ToDateTime(Global.ed[280].stop_time);
                        string ts = (t2 - t1).TotalMinutes.ToString("0.00");
                        string InsertStr6 = "insert into OEE_DT([DateTime],[ErrorCode],[EventTime],[ModuleCode],[RunStatus],[ErrorInfo],[TimeSpan])" + " " + "values(" + "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "'" + "," + "'" + Global.ed[280].errorCode + "'" + "," + "'" + Global.ed[280].start_time + "'" + ","
                           + "'" + Global.ed[280].ModuleCode + "'" + "," + "'" + Global.ed[280].errorStatus + "'" + "," + "'" + Global.ed[280].errorinfo + "'" + "," + "'" + ts + "'" + ")";
                ...
                MainFrm.Goee?.UploadDowntime(poorNum, TotalNum, "9", "20010001", "",false, EventTime, "首件手动");
                ///0924
        ...
        private void Btn_UpLoad_errortime_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                //Global.PLC_Client.WritePLC_D(23022, new short[] { 2 });//通知PLC做首件做验证已结束
            }
            catch
            { }
        ...
        private void btn_SelectModifyData_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt2 = new DataTable();
                dgv_ModifyData.DataSource = dt2;
                //dgv_ModifyData.DataSource = readCSV(System.Environment.CurrentDirectory + @"\参数修改记录\20210412.csv");
                DirectoryInfo fdir = new DirectoryInfo(System.Environment.CurrentDirectory + @"\参数修改记录\");
                FileInfo[] files = fdir.GetFiles();
                DataTable dt = new DataTable();
                int csvFileNumber = 0;
                bool ifcomplex = false;
                //标示是否是读取的第一行
                bool IsFirst = true;
                foreach (FileInfo file in files)
                {
                    string str = file.Name.Replace(".csv", "");
                    string date = str.Substring(0, 4) + "-" + str.Substring(4, 2) + "-" + str.Substring(6, 2);
                    DateTime dtime = Convert.ToDateTime(date);
                    if (dtime >= dtp_ModifyDataStartTime.Value.Date && dtime <= dtp_ModifyDataEndTime.Value.Date)
                    {
                ...
                dgv_ModifyData.DataSource = dt;
                _mainparent.dgv_AutoSize(dgv_ModifyData);
            }
            catch (Exception ex)
            {
                Log.WriteLog(ex.ToString());
        ...
        private void Btn_Start_break_Click(object sender, EventArgs e)
        {
            if (Global.j == 1 && !Global.Error_PendingStatus) //吃饭休息仅在待料时才能开始，待料时触发手选也不可以开始
            {
                ...
                MainFrm.Goee?.UploadDowntime(poorNum, TotalNum, "7", "11010001", "",false, EventTime, "吃饭休息手动");
                ///0924
                ...
                DateTime t2 = Convert.ToDateTime(Global.ed[Global.Error_PendingNum].stop_time);
                string ts = (t2 - t1).TotalMinutes.ToString("0.00");
                string InsertOEEStr = "insert into OEE_DT([DateTime],[ErrorCode],[EventTime],[ModuleCode],[RunStatus],[ErrorInfo],[TimeSpan])" + " " + "values(" + "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'" + "," + "'" + Global.ed[Global.Error_PendingNum].errorCode + "'" + "," + "'" + Global.ed[Global.Error_PendingNum].start_time + "'" + ","
                                   + "'" + "" + "'" + "," + "'" + Global.ed[Global.Error_PendingNum].errorStatus + "'" + "," + "'" + Global.ed[Global.Error_PendingNum].errorinfo + "'" + "," + "'" + ts + "'" + ")";
                SQL.ExecuteUpdate(InsertOEEStr);
                Log.WriteCSV(DateTime.Now.ToString("HH:mm:ss") + "," + Global.ed[Global.Error_PendingNum].errorCode + "," + Global.ed[Global.Error_PendingNum].start_time + "," + "自动发送成功" + "," + Global.ed[Global.Error_PendingNum].errorStatus + "," + Global.ed[Global.Error_PendingNum].errorinfo + "," + ts, @"F:\装机软件\系统配置\System_ini\");
        ...
        public void Btn_UpLoad_break_Click(object sender, EventArgs e)
        {
            ...
            if (Global.BreakStatus && Global.BreakStartTime != null)
            {
                ...
                string ts = (t2 - t1).TotalMinutes.ToString("0.00");
                string InsertOEEStr = "insert into OEE_DT([DateTime],[ErrorCode],[EventTime],[ModuleCode],[RunStatus],[ErrorInfo],[TimeSpan])" + " " + "values(" + "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'" + "," + "'" + "11010001" + "'" + "," + "'" + Global.BreakStartTime + "'" + ","
                                   + "'" + "" + "'" + "," + "'" + "7" + "'" + "," + "'" + "吃饭休息" + "'" + "," + "'" + ts + "'" + ")";
                SQL.ExecuteUpdate(InsertOEEStr);
