...
namespace 卓汇数据追溯系统
{
    public class Melsoft_PLC_TCP3
    {
        ...
        public int[] ReadPLC_DD(int address, Int16 count)
        {
            lock (client)
            {
                try
        ...
        public bool WritePLC_D(int address, short[] data)
        {
            lock (client)
            {
                try
                {
                    ...
                    string Head2 = DecToHex_Str_LH4((short)(12 + 2 * data.Length));
                    string Head3 = "100001140000";
                    string Address = DecToHex_Str_LH6(address);
                    string Command = "A8";
                    string Count = DecToHex_Str_LH4((short)data.Length);
